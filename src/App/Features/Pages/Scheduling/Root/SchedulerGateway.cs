...
{
    public async Task<Result<List<ScheduleDto>>> GetAllAsync()
    {
        var request = new HttpRequestMessage(HttpMethod.Get, "scheduling");

        return await remoteProxy.SendAsync<List<ScheduleDto>>(request);
    }
    ...
    public async Task<Result<ScheduleDto>> CreateAsync(AddScheduleDto dto)
    {
        ...
        return await remoteProxy.SendAsync<ScheduleDto>(request);
    }
