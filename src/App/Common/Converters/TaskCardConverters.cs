...
/// <summary>
/// Converts ICollection&lt;SessionDto&gt; to aggregate duration string (e.g. "Sessions: 2h 30m"),
/// or null if no sessions exist.
/// </summary>
public sealed class SessionsTotalConverter : IValueConverter
{
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (value is not ICollection<SessionDto> sessions || sessions.Count == 0)
            return null;

        var ticks = sessions.Sum(s =>
            s.Segments
                .Where(seg => seg.EndedAtUtc is not null)
                .Sum(seg => (seg.EndedAtUtc!.Value - seg.StartedAtUtc).Ticks));

        if (ticks == 0)
            return "Sessions: —";

        var total = TimeSpan.FromTicks(ticks);
        var display = total.TotalHours >= 1
