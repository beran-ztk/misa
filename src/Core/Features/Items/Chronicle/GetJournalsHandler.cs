...
public record GetChronicleQuery(DateTimeOffset From, DateTimeOffset To);

public sealed class GetChronicleHandler(IItemRepository repository, ITimeProvider timeProvider)
{
    public async Task<List<ChronicleEntryDto>> HandleAsync(GetChronicleQuery query)
    {
