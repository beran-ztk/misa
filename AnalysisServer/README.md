...
```powershell
docker build -f AnalysisServer/Dockerfile -t music-analysis:gpu .
docker run --rm -p 8000:8000 music-analysis:gpu
```
...
server. From `AnalysisServer`, create `.env` from `.env.example`, then run:
...
```powershell
docker save music-analysis:gpu -o AnalysisServer\music-analysis-gpu.tar
```

On the Linux server:

```bash
docker load -i music-analysis-gpu.tar
